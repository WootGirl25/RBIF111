...
```{r}
#age~geneexpr fitted to linear
#plot(exprs_data[probe_p.min,],age, xlab="Gene Expression", ylab="Age", main= "Age~Gene Expression fitted with linear model")
#points(min.df$G[!is.na(min.df$AGE)], predict(min.lm), col="red", pch=20)

#fitted vs observed
plot(fitted(min.lm), min.df[!is.na(min.df$AGE), "AGE"], main=paste("Plot of Fitted vs Observed Data (relationship b/w age and probe", probe_p.min), xlab="Fitted", ylab="Observed",)
abline(0,1,lty=2)
...
legend(
  ...
  pch = c(16, NA, NA), 
  lty = c(NA, 2, 2), 
  cex = 0.8  # Shrinks the legend text
)
```
