...
#cluster_matrix now contains samples as rows, and each column is a separate slice. the cells indicate which branch/cluster that sample was grouped into (1,2,3, or 4)### CHECK THIS - IS MY UNDERSTANDING HERE CORRECT????#####################

# for each sample (patient), count how many times it is assigned to its most common cluster
leaf_stability <- apply(cluster_matrix, 1, function(x) {
  max(table(x)) #for each row(patient) what is the maximum number of times it was in the same branch? Creates a contingency table and selects the maximum value.
})

# Print the stability count for each sample -leaf stability gives the most times that sample was in the same branch (cluster) across all 4 slices. if =4, means in the same branch each time.
...
#for each column (slice set) in the matrix, if the value of each row within that column is equal to the same index position of the most_common_cluster then sum these values???? ###################CHECK UNDERSTANDING HERE#############################
slice_stability <- apply(cluster_matrix, 2, function(col) {
  sum(col == most_common_cluster)
})

#Find which slice has the greatest number of stable leaves (is equal to the most common cluster the most number of times) ---UPTO HERE!!!!!!!!
